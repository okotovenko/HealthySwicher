    ...
    (() => {
        ...
        function formRating() {
            ...
            function initRatings() {
                ...
                function setRatingActiveWidth(index = ratingValue.innerHTML) {
                    const ratingActiveWidth = index / .5;
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) new core(".swiper", {
                ...
                slidesPerView: 2,
                navigation: {
                    prevEl: "._icon-arrow-prev",
                    nextEl: "._icon-arrow-next"
                },
                breakpoints: {
                    200: {
                        slidesPerView: 1,
        ...
        class ScrollWatcher {
            ...
            scrollWatcherCallback(entry, observer) {
                ...
                if ("titleAnim" === targetElement.dataset.watch && targetElement.classList.contains("_watcher-view")) {
                    let title = document.querySelectorAll("._watcher-view.title, ._watcher-view.subtitle");
                    console.log(title);
                    for (let i = 0; i < title.length; i++) {
                        let titleElement = title[i];
                        for (let j = 0, time = 0; j < titleElement.children.length; j++, time += .3) {
                            let element = titleElement.children[j];
                            element.style = `\n\t\t\t\t\topacity: 1;\n\t\t\t\t\ttransition: opacity 3s ease ${time}s;\n\t\t\t\t\t`;
                        }
                    }
                }
                if ("textAnim" === targetElement.dataset.watch && targetElement.classList.contains("_watcher-view")) {
                    let animElement = document.querySelectorAll(`._watcher-view.text, ._watcher-view.work__box,\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t._watcher-view.dishes__item, ._watcher-view.chefs__item, \n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t._watcher-view.recipes__item, ._watcher-view.recipes__left,\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t._watcher-view.social__item, ._watcher-view.items__item,\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t._watcher-view.footer__logo, ._watcher-view.footer__copy`);
                    if (animElement.length > 0) animElement.forEach((element => {
